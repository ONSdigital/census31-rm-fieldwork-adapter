...
import uk.gov.ons.census.fieldworkadapter.client.ExceptionManagerClient;
...
class ManagedMessageRecovererTest {
  ...
  private static final String SERVICE_NAME = "Fieldwork Adapter";
  ...
  public void testRecoverLogIt() {
    ...
    verify(exceptionManagerClient)
        .reportException(
            ...
            contains(
                "uk.gov.ons.census.fieldworkadapter.messaging.ManagedMessageRecovererTest.testSetupTestRecover"));
    verify(originalMessage, never()).nack();
    verify(originalMessage, never()).ack();
    assertThat(thrownException.getMessage())
        .isEqualTo("Cannot process this message at this time, but it will be retried");
  ...
  public void testRecoverSkip() {
    ...
    assertThat(skippedMessage.getService()).isEqualTo(SERVICE_NAME);
  }
